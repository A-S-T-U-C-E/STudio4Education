...
function formatChange() {
    ...
    if (document.getElementById('format').value === 'ManualDef') {
...
function updateLanguage() {
    ...
    switch (document.getElementById('format').value) {
        case 'JSON':
            ...
            document.getElementById("languageTA").value = code;
            break;
        case 'JavaScript':
...
function updateGenerator(block) {
    ...
    if (language === "ManualCode")
        language = languageOldValue;
    else
        languageOldValue = language;
    var code = [];
    ...
    var code = document.getElementById("generatorPre").textContent.trim();
    document.getElementById("generatorTA").value = code;
    if (document.getElementById('language').value === "ManualCode") {
        document.getElementById("generatorPre").style.display = "none";
        document.getElementById("generatorTA").style.display = "block";
        document.getElementById("generatorTA").focus();
    } else {
        document.getElementById("generatorPre").style.display = "block";
        document.getElementById("generatorTA").style.display = "none";
...
function updatePreview() {
    ...
    try {
        ...
        //useful to send blockType to mainWorkspace
        sessionStorage.setItem("blockFactory_blockType", blockType);
        var xmlDom = Blockly.Xml.workspaceToDom(previewWorkspace);
        var xmlText = Blockly.Xml.domToPrettyText(xmlDom);
        sessionStorage.setItem("blockFactory_previewWorkspace", xmlText);
        // compatible with Blockly v8
        // var toto = Blockly.serialization.blocks.save(
        //     blockType, { addCoordinates: true, addNextBlocks: false });
        // console.log(toto);
    } finally {
