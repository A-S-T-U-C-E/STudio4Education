...
Blockly.Xml.domToWorkspace = function(xml, workspace) {
    ...
    try {
        for (var i = 0, xmlChild;
            (xmlChild = xml.childNodes[i]); i++) {
...
Blockly.Xml.domToVariables = function(xmlVariables, workspace) {
    for (var i = 0, xmlChild;
        (xmlChild = xmlVariables.childNodes[i]); i++) {
...
Blockly.Xml.domToBlockHeadless_ = function(xmlBlock, workspace) {
    ...
    var blockChild = null;
    for (var i = 0, xmlChild;
        (xmlChild = xmlBlock.childNodes[i]); i++) {
        ...
        for (var j = 0, grandchild;
            (grandchild = xmlChild.childNodes[j]); j++) {
